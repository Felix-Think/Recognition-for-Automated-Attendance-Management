...
class Register:
    def __init__(self, root, main_ui):
        ...
        self.btn_cyan_1 = tk.Button(self.root, text="Chup", bg="#00E5FF", fg="black", font=("Arial", 12, "bold"), width=15, height=2,command=self.takePicture)
    ...
    def takePicture(self):
        """
        Lưu ảnh khuôn mặt hiện tại vào thư mục 'Images'.
        """
        ret, frame = self.cap.read()
        face_crop = self.facePose.TakePicture(frame)
        cv2.imwrite(f"{self.facePose.index}.jpg", face_crop)
        self.facePose.index += 1 # Tăng ID để lưu ảnh tiếp theo
    def on_close(self):
