...
class FaceEstimator:
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                if success:
                    ...
                    # Hiển thị thông tin head pose trên khung hình
                ...
                # Cho phép chụp ảnh khuôn mặt khi nhấn phím 't'
                key = cv2.waitKey(1) & 0xFF
                if key == ord('t'):
                    filename = f"cropped_face_{self.index}.jpg"
    ...
    def register(self):
        """
        Vòng lặp chính để đọc và xử lý từng khung hình từ camera.
        Nhấn 'q' để thoát.
        """
        while True:
            ...
            if not ret:
                break
            processed_frame = self.facePose(frame)
            # Thoát nếu nhấn 'q'
    ...
    def takeAttendance(self):
        while True:
            ...
            face_crop = self.recognization(frame)
            cv2.imshow("Take Attendance", face_crop)
